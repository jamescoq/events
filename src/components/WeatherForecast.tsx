...
const WeatherForecast = () => {
	const { data, isLoading } = useQuery<Weathertype, Error>(
		'weather',
		async () => {
			const position = await getPosition();
			const data = await fetch(
				`https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=${position?.coords?.latitude}&lon=${position?.coords?.longitude}`
